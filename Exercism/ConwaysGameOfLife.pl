...
tick(M,X,Y, 1):- pos(X,Y, M, Pos), Pos = 1, neighbors(X, Y, M, List), R2 in 2..3,
    sum_list(List, R), R2 is R - Pos.
tick(M,X,Y, 1):- pos(X,Y, M, Pos), Pos = 0, neighbors(X, Y, M, List), sum_list(List, R),
    3 is R - Pos.
tick(M,X,Y, 0):- pos(X,Y, M, Pos), Pos = 1, neighbors(X, Y, M, List), sum_list(List, R),
    R2 is R - Pos,  R2 > 3.
tick(M,X,Y, 0):- pos(X,Y, M, Pos), Pos = 1, neighbors(X, Y, M, List), sum_list(List, R),
    R2 is R - Pos,  R2 < 2.
tick(M,X,Y, 0):- pos(X,Y, M, Pos), Pos = 0, neighbors(X, Y, M, List), sum_list(List, R),
    R2 is R - Pos, R2 \= 3.
